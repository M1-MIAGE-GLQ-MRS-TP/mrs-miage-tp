 ...
 * - Statut en String au lieu d'enum
 * - java.util.Date au lieu de java.time
 * - Logique métier dans le modèle
 * - Pas d'encapsulation des règles de transition d'état
 */
public class Loan {
    ...
    // Références directes (couplage fort)
    private Member member;
    private Book book;
    
    // Dates en java.util.Date (obsolète)
    ...
    // Pénalité calculée (ne devrait pas être stockée ici)
    ...
    public Loan(String id, Member member, Book book) {
        ...
        // Calcul de la date de retour dans le constructeur (mauvais !)
    ...
    // Méthode avec logique métier (devrait être dans un service)
    ...
    // Calcul des jours de retard (logique métier dans le modèle)
    ...
    // Méthode de renouvellement (logique métier complexe dans le modèle)
    public boolean canRenew() {
        ...
        // (impossible à faire proprement ici sans accès aux réservations)
        return true;
    }
    
    // Renouvellement (modifie l'état interne - effet de bord)
    public void renew() {
        if (!canRenew()) {
